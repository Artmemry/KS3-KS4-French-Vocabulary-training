...
const CFG={
  key:"fr",
  ls:"lexique-ks-fr-v1",          // its own storage: KS3–KS4 never mixes with A-Level
  prefix:"LEXKSFR2.",             // export code prefix — the dashboard sorts by this
  ttsLang:"fr-FR",
...
const T={
  ...
  yearLabel:"Ta classe", yearAll:"Toutes",
  homeLede:"These are the word lists from your French lessons. Pick your year, then a unit, then a lesson. You can just read a list, or practise it by typing the answers. Your progress is saved on this device, and when you finish you can send it to your teacher in one click.",
  ...
  dueCard:n=>n+" mot"+(n>1?"s":"")+" à réviser aujourd'hui · "+n+" word"+(n>1?"s":"")+" to review today",
  dueMins:n=>"about "+Math.max(1,Math.round(n*12/60))+" min",
  startReview:"Lancer la révision →",
  nextNew:"Commence ici · Start here",
  nextOn:"Continue · Continue",
  nextMeta:(t,n)=>t+" · "+n+" mot"+(n>1?"s":""),
  nextGo:"Commencer →",
  clsSet:c=>"Classe : "+c, clsChange:"Changer",
  unitsLabel:"Units",
  ...
  unitMeta:(l,w,s,m)=>`${l} leçons · ${w} mots`+(m?` · ${m} maîtrisés`:"")+(s&&!m?` · ${s} vus`:""),
  yearEmpty:"No lists for that year yet.",
  lessonLine:(n,t,c)=>`Leçon ${n} — ${t} (${c} mots)`,
  lessonMeta:(s,m)=>m?`${m} maîtrisés`:(s?`${s} vus`:""),
  ...
  /* Not "épreuve": below the sixth form that reads as something marked and
     shown to someone. It is a self-check and is named as one. */
  examTab:"Teste-toi", examTitle:"Teste-toi",
  ...
  clsOfferGo:to=>"Passer en "+to, clsOfferNo:"Non, merci",
  clsLabel:"Classe", clsHint:"Optional. Your teacher's task link sets it for you; change it here if it is wrong.", clsAsk:"Which class are you in?", taskGo:"Voir la tâche", taskStrip:l=>"This week: "+l,
  progressTitle:"Suivi",
  ...
  noName:"(sans nom)",
  backupFile:"french-vocab-progress.json",
...
function renderAccueil(){
  ...
  /* Eighteen units across five years is too long a list to scroll. The year
  ...
  yearBar.append(el("button",{class:"btn small"+(shownYear?" ghost":" primary"),
    onclick:()=>{ yearFilter="__all"; openUnit=null; renderAccueil(); }}, T.yearAll));
  v.append(yearBar);
...
/* ═════════ SESSIONS ═════════ */
/* Français → Anglais is the gentler way in, so Years 7–9 start there; Years 10
